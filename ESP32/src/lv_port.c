...
// #define LV_PORT_BUFFER_MALLOC           (MALLOC_CAP_INTERNAL |
// MALLOC_CAP_8BIT)
...
void lv_port_init(void) {
  ...
  xTaskCreate(lvgl_task, "lvgl_task", 4096 * 4, NULL, CONFIG_LCD_TASK_PRIORITY,
              &lvgl_task_handle);
}
...
static void disp_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area,
   ...
   * one-by-one*/
  bsp_lcd_flush(area->x1, area->y1, area->x2 + 1, area->y2 + 1,
                (uint8_t *)color_p);
}
...
static void lv_port_disp_init(void) {
  ...
  buf1 =
      heap_caps_malloc(buffer_size * sizeof(lv_color_t), LV_PORT_BUFFER_MALLOC);
  assert(buf1);
#endif
...
static void lv_port_indev_init(void) {
  ...
  lv_indev_set_cursor(indev_touchpad,
...
static esp_err_t lv_port_tick_init(void) {
  ...
  ESP_ERROR_CHECK(
      esp_timer_start_periodic(periodic_timer, tick_inc_period_ms * 1000));
 ...
 * @note  It's useful only when enable lvgl direct-mode, and should be called
 * behind the end of flushing in lvgl.
 *
 */
static void lv_port_direct_mode_copy(void) {
