...
static esp_err_t __nvs_read_then_check(char* p_key, void* p_data,
                                       size_t r_len) {
  nvs_handle_t my_handle;
  esp_err_t err;
...
void NvmDataMgmtEvent(uint16_t notifyFlags) { NvmNotifyFlags = notifyFlags; }
...
uint16_t NvmDataMgmtStore(void) {
  ...
  if ((NvmNotifyFlags & LORAMAC_NVM_NOTIFY_FLAG_CRYPTO) ==
    ...
    esp_err_t err = __nvs_write(NVS_DATA_CRYPTO_KEY, (uint8_t*)&nvm->Crypto,
                                sizeof(nvm->Crypto));
    if (err != ESP_OK) {
  ...
  if ((NvmNotifyFlags & LORAMAC_NVM_NOTIFY_FLAG_MAC_GROUP1) ==
    ...
    esp_err_t err =
        __nvs_write(NVS_DATA_MACGROUP1_KEY, (uint8_t*)&nvm->MacGroup1,
  ...
  if ((NvmNotifyFlags & LORAMAC_NVM_NOTIFY_FLAG_MAC_GROUP2) ==
    ...
    esp_err_t err =
        __nvs_write(NVS_DATA_MACGROUP2_KEY, (uint8_t*)&nvm->MacGroup2,
  ...
  if ((NvmNotifyFlags & LORAMAC_NVM_NOTIFY_FLAG_SECURE_ELEMENT) ==
    ...
    esp_err_t err =
        __nvs_write(NVS_DATA_SECURE_KEY, (uint8_t*)&nvm->SecureElement,
  ...
  if ((NvmNotifyFlags & LORAMAC_NVM_NOTIFY_FLAG_REGION_GROUP1) ==
    ...
    esp_err_t err =
        __nvs_write(NVS_DATA_REGION1_KEY, (uint8_t*)&nvm->RegionGroup1,
  ...
  if ((NvmNotifyFlags & LORAMAC_NVM_NOTIFY_FLAG_REGION_GROUP2) ==
    ...
    esp_err_t err =
        __nvs_write(NVS_DATA_REGION2_KEY, (uint8_t*)&nvm->RegionGroup2,
  ...
  if ((NvmNotifyFlags & LORAMAC_NVM_NOTIFY_FLAG_CLASS_B) ==
    ...
    esp_err_t err = __nvs_write(NVS_DATA_CLASSB_KEY, (uint8_t*)&nvm->ClassB,
                                sizeof(nvm->ClassB));
    if (err != ESP_OK) {
...
uint16_t NvmDataMgmtRestore(void) {
  ...
  // Crypto
  err = __nvs_read_then_check(NVS_DATA_CRYPTO_KEY, (uint8_t*)&data.Crypto,
                              sizeof(data.Crypto));
  if (err == ESP_OK) {
  ...
  // Secure element
  err =
      __nvs_read_then_check(NVS_DATA_SECURE_KEY, (uint8_t*)&data.SecureElement,
                            sizeof(data.SecureElement));
  if (err == ESP_OK) {
    memcpy((uint8_t*)&nvm->SecureElement, (uint8_t*)&data.SecureElement,
           sizeof(data.SecureElement));
  } else if (err != ESP_ERR_NVS_NOT_FOUND) {
  ...
  // Mac Group 2
  err = __nvs_read_then_check(NVS_DATA_MACGROUP2_KEY, (uint8_t*)&data.MacGroup2,
                              sizeof(data.MacGroup2));
  if (err == ESP_OK) {
    if (new_region != data.MacGroup2.Region) {
      ESP_LOGI("lorawan", "change region: %d --> %d", data.MacGroup2.Region,
               new_region);
      return 0;  // When changing the region, the following fields cannot be
                 // restored.
    }

    memcpy((uint8_t*)&nvm->MacGroup2, (uint8_t*)&data.MacGroup2,
           sizeof(data.MacGroup2));
  } else if (err != ESP_ERR_NVS_NOT_FOUND) {
  ...
  // Mac Group 1
  err = __nvs_read_then_check(NVS_DATA_MACGROUP1_KEY, (uint8_t*)&data.MacGroup1,
                              sizeof(data.MacGroup1));
  if (err == ESP_OK) {
    memcpy((uint8_t*)&nvm->MacGroup1, (uint8_t*)&data.MacGroup1,
           sizeof(data.MacGroup1));
  } else if (err != ESP_ERR_NVS_NOT_FOUND) {
  ...
  // Region group 1
  err =
      __nvs_read_then_check(NVS_DATA_REGION1_KEY, (uint8_t*)&data.RegionGroup1,
                            sizeof(data.RegionGroup1));
  if (err == ESP_OK) {
    memcpy((uint8_t*)&nvm->RegionGroup1, (uint8_t*)&data.RegionGroup1,
           sizeof(data.RegionGroup1));
  } else if (err != ESP_ERR_NVS_NOT_FOUND) {
  ...
  // Region group 2
  err =
      __nvs_read_then_check(NVS_DATA_REGION2_KEY, (uint8_t*)&data.RegionGroup2,
                            sizeof(data.RegionGroup2));
  if (err == ESP_OK) {
    memcpy((uint8_t*)&nvm->RegionGroup2, (uint8_t*)&data.RegionGroup2,
           sizeof(data.RegionGroup2));
  } else if (err != ESP_ERR_NVS_NOT_FOUND) {
  ...
  // Class b
  err = __nvs_read_then_check(NVS_DATA_CLASSB_KEY, (uint8_t*)&data.ClassB,
                              sizeof(data.ClassB));
  if (err == ESP_OK) {
