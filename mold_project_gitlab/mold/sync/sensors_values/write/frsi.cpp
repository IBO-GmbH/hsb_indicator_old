#include "frsi.hpp"
#include "mold/sync/sensor_id_frsi.hpp"

using namespace mold::sync::sensors_values::write;
frsi::frsi(handler &writer, mold::options_handler &options,
           mold::configuration_handler &configurations,
           room::settings::with_sensors::handler &room_handler)
    : m_logger{"sync::sensors_values::write::frsi"},
      m_writer(writer),
      m_options(options),
      m_configurations(configurations) {
  connect_signals(room_handler);
}

void frsi::connect_signals(
    room::settings::with_sensors::handler &room_handler) {
  m_signal_connections.emplace_back(m_options.signal_frsi_updated.connect(
      [this]() { handle_box_with_catch(); }));
  const auto configuration_callback =
      [this](const configuration &configuration_) {
        handle_configuration_with_catch(configuration_);
      };
  m_signal_connections.emplace_back(
      room_handler.signal_send_fake_values.connect(configuration_callback));
}

void frsi::handle_box_with_catch() {
  try {
    handle_box();
  } catch (const std::runtime_error &error) {
    LOGGING_LOG(m_logger, logging::severity::error)
        << "failed to handle_box. error:" << error.what();
  }
}

void frsi::handle_box() {
  const auto value = m_options.get_frsi();
  const auto configurations = m_configurations.get_all();
  for (const auto &configuration : configurations) {
    if (configuration.mold_settings_ &&
        configuration.mold_settings_->building_type_ != building_type::none)
      continue;
    send(configuration.id, value);
  }
}

void frsi::handle_configuration_with_catch(
    const mold::configuration &configuration_) {
  try {
    handle_configuration(configuration_);
  } catch (const std::runtime_error &error) {
    LOGGING_LOG(m_logger, logging::severity::error)
        << "failed to handle_configuration. error:" << error.what();
  }
}

void frsi::handle_configuration(const mold::configuration &configuration_) {
  if (!configuration_.mold_settings_ ||
      configuration_.mold_settings_->building_type_ == building_type::none) {
    const auto value = m_options.get_frsi();
    send(configuration_.id, value);
    return;
  }
  const auto value = configuration_.mold_settings_->frsi;
  send(configuration_.id, value);
}

void frsi::send(const wolf::types::uuid_array &config_id, const float value) {
  LOGGING_LOG(m_logger, logging::severity::verbose)
      << "handle. id:" << config_id << ", frsi:" << value;

  const auto sensor_id = sensor_id_frsi::create(config_id);
  const auto now = std::chrono::system_clock::now();
  const wolf::sensor_value sensor_value{sensor_id, value, now};
  wolf::sensor_values values;
  values.push_back(sensor_value);
  m_writer.handle(values);
}
