...
struct wpa_ctrl *wpa_ctrl_open(const char *ctrl_path) {
  ...
  ctrl = reinterpret_cast<wpa_ctrl *>(os_malloc(sizeof(*ctrl)));
    ...
    if (FD_ISSET(ctrl->s, &rfds)) {
      ...
      if (res > 0 && reply[0] == '<') {
        ...
        if (msg_cb) {
           ...
           * terminated. */
          if ((size_t)res == *reply_len) res = (*reply_len) - 1;
          reply[res] = '\0';
          msg_cb(reply, res);
        }
        continue;
...
struct wpa_ctrl *wpa_ctrl_open(const char *ctrl_path) {
  ...
  if (ctrl_path == NULL)
    ...
    _snwprintf(name, 256, NAMED_PIPE_PREFIX TEXT("-%S"), ctrl_path);
#else  /* UNICODE */
  if (ctrl_path == NULL)
    os_snprintf(name, 256, NAMED_PIPE_PREFIX);
  else
    os_snprintf(name, 256, NAMED_PIPE_PREFIX "-%s", ctrl_path);
#endif /* UNICODE */
...
int wpa_ctrl_request(struct wpa_ctrl *ctrl, const char *cmd, size_t cmd_len,
  ...
  if (!WriteFile(ctrl->pipe, cmd, cmd_len, &written, NULL)) return -1;

  if (!ReadFile(ctrl->pipe, reply, *reply_len, &readlen, NULL)) return -1;
  *reply_len = readlen;
...
int wpa_ctrl_recv(struct wpa_ctrl *ctrl, char *reply, size_t *reply_len) {
  DWORD len = *reply_len;
  if (!ReadFile(ctrl->pipe, reply, *reply_len, &len, NULL)) return -1;
  *reply_len = len;
  return 0;
}
...
int wpa_ctrl_pending(struct wpa_ctrl *ctrl) {
  ...
  if (!PeekNamedPipe(ctrl->pipe, NULL, 0, NULL, &left, NULL)) return -1;
  return left ? 1 : 0;
}
