...
void wifi_connection::read_mqtt_settings() {
  ...
  if (wifi_server.hasArg("outPublishInterval"))
    mqtt_settings_.send_interval =
        wifi_server.arg("outPublishInterval").toInt();
}

void wifi_connection::read_sensor_settings() {
  if (wifi_server.hasArg("proximityEnable"))
    sensor_settings_.proximity_enabled = wifi_server.hasArg("proximityEnable");
  if (wifi_server.hasArg("turnOffTime"))
    sensor_settings_.display_turn_off_time =
        wifi_server.arg("turnOffTime").toInt();
  if (wifi_server.hasArg("displayInterval"))
    sensor_settings_.display_rotation_interval =
        wifi_server.arg("displayInterval").toInt();
  ...
  if (wifi_server.hasArg("pressUnit"))
    sensor_settings_.pressure_unit =
        std::string(wifi_server.arg("pressUnit").c_str());
  if (wifi_server.hasArg("tempOffset"))
    sensor_settings_.sensor_offsets[0] = wifi_server.arg("tempOffset").toDouble();
  if (wifi_server.hasArg("co2Offset"))
    sensor_settings_.sensor_offsets[1] = wifi_server.arg("co2Offset").toDouble();
  if (wifi_server.hasArg("pressOffset"))
    sensor_settings_.sensor_offsets[2] = wifi_server.arg("pressOffset").toDouble();
}